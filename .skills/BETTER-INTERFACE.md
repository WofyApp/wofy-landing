...
description: Audit interface source code in this repository and report prioritized accessibility, layout, writing, typography, and UI-polish improvements without opening a browser or modifying application code.
---
...
This skill audits only source files and configuration present in the current repository. Never open a browser, visit a local or deployed URL, start a development or preview server, capture screenshots, inspect a rendered page, or use a design canvas. Do not install dependencies or generate build artifacts for a review. Read-only source searches and static checks that do not write files are allowed.

Base every finding on code evidence with an exact `path/to/file:line` citation. When user impact depends on rendering or runtime behavior, describe it explicitly as a source-backed risk or inference; never claim it was visually or interactively observed. The report must still recommend a concrete improvement. Respect the exact component, route, or file scope requested by the user and do not audit adjacent surfaces merely because they are imported or nearby.
...
Trace the interface from the source as a keyboard-only user first: follow DOM order, native semantics, focusability, event handlers, state attributes, visibility rules, and every available state such as hover, focus, active, loading, empty, and responsive variants. Inspect declared sizes, spacing, wrapping, overflow, breakpoints, motion, and content lengths without pretending these were rendered.
...
Identify the framework, styling system, component library, design tokens, declared breakpoints, and available non-mutating static checks. For copy, inspect text within the requested scope plus an explicit terminology or content-style source when one exists; do not expand into unrelated interface surfaces merely to find comparisons.

Inspect the source implementation for every responsive variant represented in the requested scope, including desktop and mobile classes, conditional markup, scripts, and state selectors. Report prioritized, actionable improvements rather than only describing the implementation. Record rendering-dependent questions under **Not verified** instead of opening the interface.
...
Every finding cites `path/to/file:line` and shows the current implementation. Findings must be demonstrable from source semantics, selectors, tokens, control flow, or static calculations. When runtime behavior determines the final result, report the code-backed risk with appropriately qualified language and list the runtime confirmation under **Not verified**.
...
### Design annotations are out of scope

Do not create canvas or frame annotations. This skill always returns the repository-code findings table as text.
...
Run only safe, non-mutating static checks already available in the project. Do not install dependencies, build the site, start a server, or open a browser. List inspected files, searches, calculations, and static checks with their results. Put rendering- or interaction-dependent confirmation under **Not verified**; never present it as completed verification.
...
| Visual claim presented as observed from source alone | Qualify it as a code-backed risk and put runtime confirmation under `Not verified` |
...
During an audit, inspect the initial-state logic and record full-refresh appearance as **Not verified**. If the user later requests implementation, runtime verification belongs to that separate task and is not authorized by this skill.
...
- Inspect each icon's declared smallest size (often `16px`) and flag excessive path detail as a source-backed recognizability risk; record actual rendered recognizability as **Not verified**.
